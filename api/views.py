...
def library(request, select=None):
	...
	if request.method == 'GET':
		documents = EpubArchive.objects.only('id', 'title', 'orderable_author').filter(user_archive__user=request.user).order_by(settings.DEFAULT_ORDER_FIELD).distinct()
...
def api_book_meta(request, key):
	if request.method == 'GET':
		document = EpubArchive.objects.only('id', 'title', 'orderable_author').get(id=key)
...
def main(request, SSL=True):
    ...
    if request.method == 'GET':
        ...
        documents = EpubArchive.objects.filter(user_archive__user=request.user).order_by(settings.DEFAULT_ORDER_FIELD).distinct()
    ...
    # Accept an epub file either by URL or by direct POST file upload with epub bytes as `epub_data`
        ...
        if 'epub_url' in request.POST:
            ...
            if form.is_valid():
                ...
                document = EpubArchive.objects.create(name=document_name)
                document.save()
        ...
        if isinstance(resp, EpubArchive):
